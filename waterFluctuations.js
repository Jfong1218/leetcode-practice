...
// for (i = 1; i < waterLevels.length; i++) {
//     if (waterLevels[i] - low > 0) {
//     if (waterLevels[i] - low > diff) {
...
const measureWaterLevels = (waterLevels) => {
    let diff = -1
    let low = waterLevels[0]
    for (i = 1; i < waterLevels.length; i++) {
        if (waterLevels[i] - low > 0 && waterLevels[i] - low > diff ) {
            diff = waterLevels[i] - low
            }
    if (waterLevels[i]-low < 0){
        low = waterLevels[i]
