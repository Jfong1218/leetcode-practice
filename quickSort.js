...
let pivot = nums[right]; //pivot = 3
let i = left - 1
for (let j = left; j < right - 1; j++) {
    if (nums[j] <= pivot) {
    i++
    [nums[i], nums[j]] = [nums[j], nums[i]] //swapping i and j

    }
}
[nums[i + 1], nums[right]] = [nums[right], nums[i + 1]] //swapping at indexes right and i+1
console.log(nums) //logs our function 
return (i + 1) //returns our pivot
}
